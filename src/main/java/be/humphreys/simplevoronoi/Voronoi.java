...
public class Voronoi {
  ...
  private Edge bisect(Site s1, Site s2) {
    ...
    adx = dx > 0 ? dx : -dx;
    ady = dy > 0 ? dy : -dy;
    newedge.c = (double) (s1.coord.x * dx + s1.coord.y * dy + (dx * dx + dy * dy) * 0.5);// get the
                                                                                         // slope of
                                                                                         // the line
  ...
  /*
   * implicit parameters: nsites, sqrt_nsites, xmin, xmax, ymin, ymax, deltax, deltay (can all be
   * estimates). Performance suffers if they are wrong; better to make nsites, deltax, and deltay
